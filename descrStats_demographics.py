...
def run_demographics(df):
    ...
    sns.distplot(df['age_C'][df['gender'] == 1])
    plt.show()
    print 'age distribution - male:'
    sns.distplot(df['age_C'][df['gender'] == 2])
    plt.show()
    
    print '\n\n#### young cohort ####\n'  
    print 'N = ' + str(len(df[df['age_C']<=40]))
    print 'N female = ' + str(len(df[(df['age_C']<=40) & (df['gender']==1)]))
    print 'N male = ' + str(len(df[(df['age_C']<=40) & (df['gender']==2)]))
    sns.countplot(df[df['age_C']<=40]['gender'])
    plt.show()
    print 'age distribution - female:'
    sns.distplot(df['age_C'][(df['age_C']<=40) & (df['gender'] == 1)])
    plt.show()
    print 'age distribution - male:'
    sns.distplot(df['age_C'][(df['age_C']<=40) & (df['gender'] == 2)])
    plt.show()